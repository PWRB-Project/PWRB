...
*Setup instructions for a gitian build of PWRB Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the PWRB
...
to the PWRB GitHub release page.
...
- [Building PWRB Core](#building-pwrb-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/pwrb-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/pwrb-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/pwrb-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/pwrb-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/pwrb-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/pwrb-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/PWRB-Project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@pwrb.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/pwrb-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/pwrb-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/pwrb-osx-*-build.assert
```
