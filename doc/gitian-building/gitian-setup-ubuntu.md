...
The build script we'll be using is contained in the PWRB github repository ([contrib/gitian-build.py](https://github.com/PWRB-Project/pwrb/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the PWRB repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/PWRB-Project/PWRB/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, pwrb-detached-sigs, and pwrb GitHub repos.
