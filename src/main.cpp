...
#if defined(NDEBUG)
#error "PWRB cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in upwrb) are considered zero fee (for relaying and mining)
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zPWRB spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!txIn.IsZerocoinPublicSpend())
                    return state.Invalid(error("%s: failed for tx %s, every input must be a zcpublicspend",
                            __func__, tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zpwrb");

                libzerocoin::ZerocoinParams* params = consensus.Zerocoin_Params(false);
                PublicCoinSpend publicSpend(params);
                if (!ZPWRBModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                    return false;
                }
                ...
                if (!CheckPublicCoinSpendVersion(publicSpend.getVersion())) {
                    return state.Invalid(error("%s : Public Zerocoin spend version %d not accepted. must be version %d.",
                            __func__, publicSpend.getVersion(), CurrentPublicCoinSpendVersion()), REJECT_INVALID, "bad-txns-invalid-zpwrb");
                }
            ...
            // Reject legacy zPWRB mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zPWRB mint output in tx %s",
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZPWRBModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    //Track zPWRB money supply
    if (!UpdateZPWRBSupplyDisconnect(block, pindex))
        return error("%s: Failed to calculate new zPWRB supply", __func__);
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from PWRB
...
{
    util::ThreadRename("pwrb-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    std::vector<CBetOut> vExpectedLottoPayouts;

    // Get the PL and CG bet payout TX's so we can calculate the winning bet vector which is used to mint coins and payout bets.
    vExpectedLottoPayouts = GetBetPayouts(pindex->nHeight);

    // Get the total amount of PWR that needs to be minted to payout all winning bets.
    CAmount nExpectedMint2 = nExpectedMint;
    nExpectedMint += GetBlockPayouts(vExpectedLottoPayouts);

    if (!IsBlockPayoutsValid(vExpectedLottoPayouts, block, pindex->nHeight) && vExpectedLottoPayouts.size() > 0) {
        if (Params().NetworkID() == CBaseChainParams::TESTNET) {
            ...
            MilliSleep(180000);
            vExpectedLottoPayouts.clear();
            vExpectedLottoPayouts = GetBetPayouts(pindex->nHeight);
            nExpectedMint = nExpectedMint2 + GetBlockPayouts(vExpectedLottoPayouts);
            if (!IsBlockPayoutsValid(vExpectedLottoPayouts, block, pindex->nHeight) && vExpectedLottoPayouts.size() > 0) {
    ...
    LogPrintf("ConnectBlock() - Expected Mint After Adding Bets to be Paid: %i\n", nExpectedMint);
    ...
    // Update zPWRB money supply map
    if (!UpdateZPWRBSupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zPWRB supply for block=%s height=%d", __func__,
                                    block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    }

    // A one-time event where the zPWRB supply was off (due to serial duplication off-chain on main net)
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculatePWRBSupply(consensus.height_start_ZC, false);
    }
    ...
    // Update PWRB money supply
    nMoneySupply += (nValueOut - nValueIn - nValueBet);
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zPWRB spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zPWRB serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Pwrb Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nPwrbBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nPwrbBadBlockBits)) {
            // accept PWRB block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> pwrbInputs;
        std::vector<CTxIn> zPWRBInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zPWRBInputs.push_back(stakeIn);
            }else{
                pwrbInputs.push_back(stakeIn);
            }
        }
        const bool hasPWRBInputs = !pwrbInputs.empty();
        const bool hasZPWRBInputs = !zPWRBInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasPWRBInputs) {
                    ...
                    for (const CTxIn& pwrbIn : pwrbInputs)
                        if(pwrbIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zPWRB inputs.
            if(hasZPWRBInputs) {
                for (const CTxIn& zPwrbInput : zPWRBInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zPwrbInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zpwrb");
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // PWRB: We use certain sporks during IBD, so check to see if they are
