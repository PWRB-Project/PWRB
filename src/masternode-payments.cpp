...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZPWRBStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (sporkManager.IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZPWRBStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZPWRBStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
    ...
    std::vector<CBetOut> vLottoPayouts;
    // Get the lotto bet payout TX's so we can calculate the winning bet vector which is used to mint coins and payout bets.
    vLottoPayouts = GetBetPayouts(pindexPrev->nHeight + 1);
    int64_t nBetPayout = GetBlockPayouts(vLottoPayouts);
    unsigned int i = txNew.vout.size();
    txNew.vout.resize(i + vLottoPayouts.size());

    for (const CTxOut& out : vLottoPayouts) {
        LogPrint(BCLog::MASTERNODE,"Adding bet payment of %s to %s to block \n", FormatMoney(out.nValue).c_str(), ScriptToAsmStr(out.scriptPubKey).c_str());
        txNew.vout[i].scriptPubKey = out.scriptPubKey;
        txNew.vout[i].nValue = out.nValue;
        i++;
    }

    // Clear vector after a payout.
    vLottoPayouts.clear();
}
