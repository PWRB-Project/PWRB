...
void AskPassphraseDialog::accept()
{
    ...
    case Mode::Encrypt: {
        ...
        bool ret = openStandardDialog(
                ...
                " <b>" + tr("LOSE ALL OF YOUR PWRB") + "</b>!<br><br>" + tr("Are you sure you wish to encrypt your wallet?"),
                tr("ENCRYPT"), tr("CANCEL")
        );
        if (ret) {
            if (newpass1 == newpass2) {
                newpassCache = newpass1;
                PWRBGUI* window = static_cast<PWRBGUI*>(parentWidget());
...
{
    PWRBGUI* gui = static_cast<PWRBGUI*>(parentWidget());
...
{
    hide();
    static_cast<PWRBGUI*>(parentWidget())->showHide(true);
    openStandardDialog(
            tr("Wallet encrypted"),
            "<qt>" +
            tr("PWRB will close now to finish the encryption process. "
               "Remember that encrypting your wallet cannot fully protect "
               "your PWRBs from being stolen by malware infecting your computer.") +
            "<br><br><b>" +
