...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::PWRB);
    ...
    if (settings.contains("nAnonymizePwrbAmount") || reset)
        SoftSetArg("-anonymizepwrbamount", settings.value("nAnonymizePwrbAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in pwrb.cpp during shotdown to also remove the window positions
