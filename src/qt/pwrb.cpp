...
#include "qt/pwrb/splash.h"
#include "qt/pwrb/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("pwrb-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in pwrb.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in pwrb.qrc)
...
/** Class encapsulating PWRB Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (PWRBGUI)
...
{
    window = new PWRBGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &PWRBGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &PWRBGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(PWRBGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(PWRBGUI::DEFAULT_WALLET);
        ...
        // PWRB: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &PWRBGUI::handlePaymentRequest);
        connect(window, &PWRBGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", PWRBGUI::tr("A fatal error occurred. PWRB can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(pwrb_locale);
    Q_INIT_RESOURCE(pwrb);
    ...
    /// 6. Determine availability of data directory and parse pwrb.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!boost::filesystem::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("PWRB Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("PWRB Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // pwrb: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("PWRB Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
