...
#include "qt/pwrb/forms/ui_addresseswidget.h"
#include "qt/pwrb/addresslabelrow.h"
#include "qt/pwrb/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(PWRBGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter PWRB address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress pwrbAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(pwrbAdd.Get(), label.toUtf8().constData(),
                pwrbAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
