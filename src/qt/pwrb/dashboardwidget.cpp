...
#include "qt/pwrb/dashboardwidget.h"
#include "qt/pwrb/forms/ui_dashboardwidget.h"
#include "qt/pwrb/sendconfirmdialog.h"
#include "qt/pwrb/txrow.h"
...
DashboardWidget::DashboardWidget(PWRBGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelPwrb, "text-chart-pwrb");
    setCssProperty(ui->labelZpwrb, "text-chart-zpwrb");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &PWRBGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &PWRBGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &PWRBGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isPwrb = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZPWRB;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair PWRB, zPWRB
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            pwrb = (pair.first != 0) ? pair.first / 100000000 : 0;
            zpwrb = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalPwrb += pair.first;
            chartData->totalZpwrb += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesPwrb.append(pwrb);
        chartData->valueszPwrb.append(zpwrb);

        int max = std::max(pwrb, zpwrb);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesPwrb);
    set1->append(chartData->valueszPwrb);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalPwrb > 0 || chartData->totalZpwrb > 0) {
        setCssProperty(ui->labelAmountPwrb, "text-stake-pwrb");
        setCssProperty(ui->labelAmountZpwrb, "text-stake-zpwrb");
    } else {
        setCssProperty(ui->labelAmountPwrb, "text-stake-pwrb-disable");
        setCssProperty(ui->labelAmountZpwrb, "text-stake-zpwrb-disable");
    }
    forceUpdateStyle({ui->labelAmountPwrb, ui->labelAmountZpwrb});
    ui->labelAmountPwrb->setText(GUIUtil::formatBalance(chartData->totalPwrb, nDisplayUnit));
    ui->labelAmountZpwrb->setText(GUIUtil::formatBalance(chartData->totalZpwrb, nDisplayUnit, true));
