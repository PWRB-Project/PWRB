...
namespace GuiTransactionsUtils {
    ...
    {
        parent->ask(parent->tr("Debugging!"), parent->tr("Inside ProcessSendCoinsReturnAndInform.\n\nDo you want to continue?"));
        CClientUIInterface::MessageBoxFlags informType;
        QString informMsg = ProcessSendCoinsReturn(parent, sendCoinsReturn, walletModel, informType, msgArg, fPrepare);
        parent->ask(parent->tr("Debugging!"), parent->tr("Inside ProcessSendCoinsReturnAndInform: informMsg - "+informMsg+"\n\nDo you want to continue?"));
        if (!informMsg.isEmpty()) parent->emitMessage(parent->translate("Send Coins"), informMsg, informType, 0);
        parent->ask(parent->tr("Debugging!"), parent->tr("Leaving ProcessSendCoinsReturnAndInform.\n\nDo you want to continue?"));
    }
