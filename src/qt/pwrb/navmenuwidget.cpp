...
#include "qt/pwrb/navmenuwidget.h"
#include "qt/pwrb/forms/ui_navmenuwidget.h"
...
NavMenuWidget::NavMenuWidget(PWRBGUI *mainWindow, QWidget *parent) :
    ...
    ui(new Ui::NavMenuWidget)
{
    ...
    ui->btnBet->setProperty("name", "bet");
    ui->btnBet->setText("BET\n");
    ui->btnBet->setToolButtonStyle(Qt::ToolButtonTextUnderIcon);
    ...
    btns = {ui->btnDashboard, ui->btnBet, ui->btnSend, ui->btnReceive, ui->btnAddress, ui->btnColdStaking, ui->btnSettings, ui->btnColdStaking};
...
void NavMenuWidget::connectActions() {
    ...
    connect(ui->btnBet, &QPushButton::clicked, this, &NavMenuWidget::onBetClicked);
    ...
    ui->btnBet->setShortcut(QKeySequence(SHORT_KEY + Qt::Key_2));
    ui->btnSend->setShortcut(QKeySequence(SHORT_KEY + Qt::Key_3));
...
void NavMenuWidget::onBetClicked(){
    window->goToBet();
    onNavSelected(ui->btnBet);
}
