...
#include "qt/pwrb/privacywidget.h"
#include "qt/pwrb/forms/ui_privacywidget.h"
#include "qt/pwrb/qtutils.h"
#include "guiutil.h"
#include "qt/pwrb/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(PWRBGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zPWRB anonymizes your PWRB by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zPWRB or convert back to PWRB"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 PWRB ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zPWRB");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zPWRB");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zPWRB");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zPWRB");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zPWRB");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zPWRB");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zPWRB");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zPWRB");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zPWRB Movements"));
    ...
    // Only Convert to PWRB enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzPWRB->setTitleClassAndText("btn-title-grey", "Total 0 zPWRB");
    ui->btnTotalzPWRB->setSubTitleClassAndText("text-subtitle", "Show denominations of zPWRB owned.");
    ui->btnTotalzPWRB->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select PWRB outputs to mint into zPWRB.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zPWRB");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzPWRB, &OptionButton::clicked, this, &PrivacyWidget::onTotalZpwrbClicked);
...
void PrivacyWidget::onMintSelected(bool isMint)
{
    QString btnText;
    if (isMint) {
        btnText = tr("Mint zPWRB");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of PWRB to mint into zPWRB"));
    } else {
        btnText = tr("Convert back to PWRB");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zPWRB to convert back into PWRB"));
    }
...
void PrivacyWidget::onTotalZpwrbClicked()
{
    bool isVisible = ui->layoutDenom->isVisible();
    if (!isVisible) {
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzPWRB->setRightIconClass("btn-dropdown", true);
    } else {
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzPWRB->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zPWRB is currently undergoing maintenance"));
        return;
    }
    ...
    if (!ctx.isValid()) {
        inform(tr("You need to unlock the wallet to be able to %1 zPWRB").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value)
{
    ...
    if (!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)) {
        ...
        // Mint succeed
        inform(tr("zPWRB minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value)
{
    ...
    if (!walletModel->convertBackZpwrb(
            value,
            selectedMints,
            receipt
    )) {
        ...
        // Spend succeed
        inform(tr("zPWRB converted back to PWRB"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply()
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zPWRB </b>";
    ...
    ui->btnTotalzPWRB->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
