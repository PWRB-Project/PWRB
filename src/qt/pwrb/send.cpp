...
#include "qt/pwrb/forms/ui_send.h"
...
SendWidget::SendWidget(PWRBGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
...
void SendWidget::refreshAmounts()
{
    ...
    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZpwrb));
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend PWRB (not zPWRB)
...
bool SendWidget::sendZpwrb(QList<SendCoinsRecipient> recipients)
{
    if (!walletModel || !walletModel->getOptionsModel())
        return false;
...
void SendWidget::onPWRBSelected(bool _isPWRB)
{
    isPWRB = _isPWRB;
