...
#include "qt/pwrb/forms/ui_sendconfirmdialog.h"
...
void TxDetailDialog::setData(WalletModel *model, const QModelIndex &index){
    ...
    if(tx) {
        ...
        if (address[0].isDigit()) {
            ...
            ui->labelSend->setText("Sending to");
            ui->textSendLabel->setText(address);
        } else {
            ui->labelSend->setText("Sending to");
...
void TxDetailDialog::setData(WalletModel *model, WalletModelTransaction &tx)
{
    ...
    if (tx.getRecipients().at(0).address[0].isDigit()) {
        if(ui->btnSave->isVisible()){
            ...
            ui->btnSave->setText(tr("BET"));
            ui->labelSend->setText("Your Bet: ");
            ui->textSendLabel->setText(tx.getRecipients().at(0).address);
        }
        ui->textSend->setVisible(false);
    } else if (nRecipients == 1) {
...
{
    ask(tr("Debugging!"), tr("Entered TxDetailDialog::acceptTx.\n\nDo you want to continue?"));
    if (!isConfirmDialog)
        throw GUIException(strprintf("%s called on non confirm dialog", __func__));
    this->confirm = true;
    ask(tr("Debugging!"), tr("Calling model->sendCoins.\n\nDo you want to continue?"));
    this->sendStatus = model->sendCoins(*this->tx);
    ask(tr("Debugging!"), tr("Calling accept().\n\nDo you want to continue?"));
    accept();
    ask(tr("Debugging!"), tr("Left accept().\n\nDo you want to continue?"));
}
