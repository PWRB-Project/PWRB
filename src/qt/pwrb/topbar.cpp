...
#include "qt/pwrb/topbar.h"
#include "qt/pwrb/forms/ui_topbar.h"
#include "qt/pwrb/lockunlock.h"
#include "qt/pwrb/qtutils.h"
#include "qt/pwrb/receivedialog.h"
#include "qt/pwrb/loadingdialog.h"
...
TopBar::TopBar(PWRBGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    std::initializer_list<QWidget*> lblTitles = {ui->labelTitle1, ui->labelTitleAvailablezPwrb, ui->labelTitle3, ui->labelTitle4, ui->labelTitlePendingzPwrb, ui->labelTitleImmaturezPwrb};
    ...
    setCssProperty({ui->labelAmountTopPwrb, ui->labelAmountTopzPwrb}, "amount-small-topbar");
    setCssProperty({ui->labelAmountPwrb, ui->labelAvailablezPwrb}, "amount-topbar");
    setCssProperty({ui->labelPendingPwrb, ui->labelPendingzPwrb, ui->labelImmaturePwrb, ui->labelImmaturezPwrb}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("PWRB")
    updateDisplayUnit();
                            ...
                            const CAmount& delegatedBalance, const CAmount& coldStakedBalance)
{
    ...
    // PWRB Total
    CAmount pwrbAvailableBalance = balance;
    // zPWRB Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalPwrb = GUIUtil::formatBalance(pwrbAvailableBalance, nDisplayUnit);
    QString totalzPwrb = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);

    // PWRB
    // Top
    ui->labelAmountTopPwrb->setText(totalPwrb);
    // Expanded
    ui->labelAmountPwrb->setText(totalPwrb);
    ui->labelPendingPwrb->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelImmaturePwrb->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));

    // Update display state and/or values for zPWRB balances as necessary
    bool fHaveZerocoins = zerocoinBalance > 0;

    // Set visibility of zPWRB label titles/values
    ui->typeSpacerTop->setVisible(fHaveZerocoins);
    ui->typeSpacerExpanded->setVisible(fHaveZerocoins);
    ui->labelAmountTopzPwrb->setVisible(fHaveZerocoins);
    ui->zerocoinBalances->setVisible(fHaveZerocoins);

    // Top
    ui->labelAmountTopzPwrb->setVisible(false);
    // Expanded
    ui->labelAvailablezPwrb->setText(totalzPwrb);
    ui->labelPendingzPwrb->setVisible(false);
    ui->labelImmaturezPwrb->setVisible(false);
    ui->labelTitleAvailablezPwrb->setVisible(false);
    ui->labelTitlePendingzPwrb->setVisible(false);
    ui->labelTitleImmaturezPwrb->setVisible(false);
}
