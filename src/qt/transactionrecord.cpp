...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        libzerocoin::CoinSpend zcspend = wtx.HasZerocoinPublicSpendInputs() ? ZPWRBModule::parseCoinSpend(wtx.vin[0]) : TxInToZerocoinSpend(wtx.vin[0]);
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zpwrbTracker->HasMintTx(hash))) {
            //zPWRB stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZPWRB;
            ...
            if (wtx.HasP2CSOutputs() && nNet <= 150000000) {
                sub.credit = nCredit;
                sub.debit = -nDebit;
                loadHotOrColdStakeOrContract(wallet, wtx, sub);
                parts.append(sub);
                return parts;
            } else if (nNet <= 150000000) {
                // PWRB stake reward
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime, wtx.GetTotalSize());
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_zPwrb;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address)) {
                    ...
                    if (scriptPubKey.length() > 0 && strncmp(scriptPubKey.c_str(), "OP_RETURN", 9) == 0) {
                        betTx = true;
                        sub.type = TransactionRecord::Bet;
                        ...
                        if(CLottoBet::FromOpCode(opCode, lb)) {
                        	std::string sDate = std::to_string(lb.nDate);
                        	if (sDate.length() >= 8) {
                        		sub.address = sDate.substr(4,2) + "-" + sDate.substr(6,2) + " : ";
                        	} else {
                        		sub.address = sDate + " : Number(s) - ";
                        	}
                        	if (lb.nNumber1 > 0)
                        		sub.address += std::to_string(lb.nNumber1) + " ";
                        	if (lb.nNumber2 > 0)
                        		sub.address += std::to_string(lb.nNumber2) + " ";
                        	if (lb.nNumber3 > 0)
                        		sub.address += std::to_string(lb.nNumber3);
                        	sub.bet = sub.address;
                        }
                    }
                }
...
bool IsZPWRBType(TransactionRecord::Type type)
{
    switch (type) {
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    // For generated transactions, determine maturity
            ...
            type == TransactionRecord::StakeZPWRB ||
...
{
    return (type == TransactionRecord::StakeMint || type == TransactionRecord::Generated || type == TransactionRecord::StakeZPWRB);
}
