...
bool WalletModel::validateBet(const QString& address)
{
    QRegExp rx("^((6[0-9]|[1-6][0-9]|[1-9]) {1}){0,2}(6[0-9]|[1-6][0-9]|[1-9])$");
    return rx.exactMatch(address);
}

CScript WalletModel::parseBet(const QString& address)
{
    QStringList numbers = address.split(' ',QString::SkipEmptyParts);
	...
	if (numbers.size() > 2)
        nNumber1 = numbers.at(2).toInt();
	if (numbers.size() > 1)
        nNumber2 = numbers.at(1).toInt();
	if (numbers.size() > 0)
        nNumber3 = numbers.at(0).toInt();

	if (nNumber1 > nNumber2) {
		nTempNumber = nNumber2;
		nNumber2 = nNumber1;
		nNumber1 = nTempNumber;
	}
	if (nNumber2 > nNumber3) {
		nTempNumber = nNumber3;
		nNumber3 = nNumber2;
		nNumber2 = nTempNumber;
	}
	if (nNumber1 > nNumber2) {
		nTempNumber = nNumber2;
		nNumber2 = nNumber1;
    ...
    for(std::string::size_type i = 0; i < unHexedOpCode.size(); ++i) {
        lottoClassData.push_back(unHexedOpCode[i]);
    }
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl, bool fIncludeDelegations)
{
    ...
    Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered pwrb address / amount:
            CScript scriptPubKey;
            CBitcoinAddress out = CBitcoinAddress(rcp.address.toStdString());
            std::string betScriptPubKey = rcp.scriptPubKey.ToString();
            bool isBet = false;

            if (betScriptPubKey.length() > 0 && strncmp(betScriptPubKey.c_str(), "OP_RETURN", 9) == 0) {
                isBet = true;
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            Q_EMIT message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 PWRB.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
bool WalletModel::createZpwrbSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZpwrb(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        CZerocoinSpendReceipt &receipt)
{
...
static void NotifyzPWRBReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzPWRBReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzPWRBReset.connect(boost::bind(NotifyzPWRBReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzPWRBReset.disconnect(boost::bind(NotifyzPWRBReset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose)
{
    ...
    if (address.IsStakingAddress())
        return error("Invalid PWRB address, cold staking address");
    CKeyID keyID;
...
{
    if (!address.IsValid())
        return error("Invalid PWRB address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from PWRB address");
...
{
    setMints.clear();
    setMints = pwalletMain->zpwrbTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
