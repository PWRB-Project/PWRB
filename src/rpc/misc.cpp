...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total pwrb balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zPWRBsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zPWRB denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zPWRB denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zPWRB denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zPWRB denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zPWRB denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zPWRB denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zPWRB denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zPWRB denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zPWRB denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,      (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,          (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,        (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in pwrb/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in pwrb/kb\n"
    ...
    UniValue zpwrbObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        if (mapZerocoinSupply.empty())
            zpwrbObj.push_back(Pair(std::to_string(denom), ValueFromAmount(0)));
        else
            zpwrbObj.push_back(Pair(std::to_string(denom), ValueFromAmount(mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zpwrbObj.push_back(Pair("total", ValueFromAmount(GetZerocoinSupply())));
    obj.push_back(Pair("zPWRBsupply", zpwrbObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"pwrbaddress\"\n"
            "\nReturn information about the given pwrb address.\n"

            "\nArguments:\n"
            "1. \"pwrbaddress\"     (string, required) The pwrb address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"pwrbaddress\",    (string) The pwrb address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for PWRB cold staking\n"
...
#ifdef ENABLE_WALLET
        // Case 1: PWRB address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are pwrb addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) pwrb address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"pwrbaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"pwrbaddress\"  (string, required) The pwrb address to use for the signature.\n"
...
UniValue getstakingstatus(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"staking_enabled\": true|false,     (boolean) whether staking is enabled/disabled in pwrb.conf\n"
            "  \"coldstaking_enabled\": true|false, (boolean) whether cold-staking is enabled/disabled in pwrb.conf\n"
            "  \"haveconnections\": true|false,     (boolean) whether network connections are present\n"
            "  \"mnsync\": true|false,              (boolean) whether the required masternode/spork data is synced\n"
            "  \"walletunlocked\": true|false,      (boolean) whether the wallet is unlocked\n"
            "  \"stakeablecoins\": n                (numeric) number of stakeable UTXOs\n"
            "  \"stakingbalance\": d                (numeric) PWRB value of the stakeable coins (minus reserve balance, if any)\n"
