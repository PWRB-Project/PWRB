...
std::vector<CSporkDef> sporkDefs = {
    ...
    MAKE_SPORK_DEF(SPORK_5_MAX_VALUE,                       1000),          // 1000 PWRB
    ...
    MAKE_SPORK_DEF(SPORK_16_ZEROCOIN_MAINTENANCE_MODE,      1584552673ULL), // OFF
    ...
    MAKE_SPORK_DEF(SPORK_19_DRAW_RESULT,                    201901010102030405), // 1st January 2019, Numbers 01, 02, 03, 04, 05 - set weekly and used as a last resort for checking bets
    MAKE_SPORK_DEF(SPORK_20_TRAVERSE_SZ,                    0), // Traversing of blockchain for results can be increased beyond the default if, for some reason, a winning ticket isn't processed
};
...
// PWRB: on startup load spork values from previous session if they exist in the sporkDB
void CSporkManager::LoadSporksFromDB()
{
    for (const auto& sporkDef : sporkDefs) {
        ...
        if (spork.nValue > 1000000 && spork.nSporkID != 10019) {
