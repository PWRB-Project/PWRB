...
bool CPwrbStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CPwrbStake from zerocoin spend");
...
bool CPwrbStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CPwrbStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CPwrbStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CPwrbStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CPwrbStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CPwrbStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CPwrbStake::GetUniqueness() const
{
    //The unique identifier for a PWRB stake is the outpoint
...
CBlockIndex* CPwrbStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CPwrbStake::ContextCheck(int nHeight, uint32_t nTime)
{
