...
/**
 * Fees smaller than this (in upwrb) are considered zero fee (for transaction creation)
        ...
        if (zwalletMain) {
            ...
            if (CWalletDB(strWalletFile).ReadCurrentSeedHash(hashSeed)) {
                ...
                if (!GetDeterministicSeed(hashSeed, nSeed)) {
                    return error("Failed to read zPWRB seed from DB. Wallet is probably corrupt.");
                }
...
CAmount CWalletTx::GetUnlockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        if (fMasterNode && vout[i].nValue == 10 * COIN) continue; // do not count MN-like outputs
...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        else if (fMasterNode && vout[i].nValue == 10 * COIN) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
        }
    ...
    for (unsigned int i = 0; i < vout.size(); ++i) {
        ...
        if (txout.IsZerocoinMint()) {
            ...
            if (!IsCoinStake() && !IsCoinBase()) {
                LogPrintf("CWalletTx::GetAmounts: Bet Placed, txid %s\n", this->GetHash().ToString());
            }
            address = CNoDestination();
        }
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    bool fCheckZPWRB = GetBoolArg("-zapwallettxes", false);
    if (fCheckZPWRB)
        zpwrbTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zpwrb
            if (fCheckZPWRB && pindex->nHeight >= Params().GetConsensus().height_start_ZC) {
                             ...
                             ) const
{
...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            if (nCoinType == STAKEABLE_COINS && nDepth <= Params().GetConsensus().nStakeMinDepth && chainActive.Height() > Params().GetConsensus().height_start_TimeProtoV2) continue;
            if (nCoinType == STAKEABLE_COINS && nDepth <= Params().GetConsensus().nCoinbaseMaturity && chainActive.Height() <= Params().GetConsensus().height_start_TimeProtoV2) continue;
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 PWRB collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 PWRB collateral
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                std::vector<unsigned char> lottoClassData;
                for(std::string::size_type i = 0; i < opReturn.size(); ++i) {
                        lottoClassData.push_back(opReturn[i]);
                }
                CScript pubSubScript = CScript() << OP_RETURN << lottoClassData;
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-pwrb-address
                    CScript scriptChange;
...
bool CWallet::CreateTransaction(CScript scriptPubKey, const CAmount& nValue, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX, CAmount nFeePay, bool fIncludeDelegated, const std::string& opReturn)
{
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptPubKey, nValue));
    return CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, strFailReason, coinControl, coin_type, useIX, nFeePay, fIncludeDelegated, opReturn);
}
        ...
        )
{
    ...
    // Parse utxos into CPwrbStakes
    std::list<std::unique_ptr<CStakeInput> > listInputs;
    for (const COutput &out : vCoins) {
        std::unique_ptr<CPwrbStake> input(new CPwrbStake());
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (nBytes >= DEFAULT_BLOCK_MAX_SIZE * 0.9)
