...
#include "zpwrb/zpwrbmodule.h"
#include "zpwrb/zpwrbwallet.h"
#include "zpwrb/zpwrbtracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_PWRB = 61000, // inherited version..
...
// Possible states for zPWRB send
enum ZerocoinSpendStatus {
    ZPWRB_SPEND_OKAY = 0,                            // No error
    ZPWRB_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZPWRB_WALLET_LOCKED = 2,                         // Wallet was locked
    ZPWRB_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZPWRB_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZPWRB_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZPWRB_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZPWRB_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZPWRB_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZPWRB_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZPWRB_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZPWRB_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZPWRB_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZPWRB_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZPWRB_SPENT_USED_ZPWRB = 14,                      // Coin has already been spend
    ZPWRB_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZPWRB_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    /// Get 10000 PWRB output and keys which can be used for the Masternode
    ...
    bool CreateZPWRBOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zPWRB wallet
    CzPWRBWallet* zwalletMain{nullptr};
    std::unique_ptr<CzPWRBTracker> zpwrbTracker{nullptr};
    void setZWallet(CzPWRBWallet* zwallet);
    CzPWRBWallet* getZWallet();
    ...
    // zPWRB reset
    boost::signals2::signal<void()> NotifyzPWRBReset;
