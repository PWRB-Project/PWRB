...
public:
    ...
    bool WriteZPWRBSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZPWRBSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZPWRBSeed_deprecated(uint256& seed);
    bool EraseZPWRBSeed();
    bool EraseZPWRBSeed_deprecated();

    bool WriteZPWRBCount(const uint32_t& nCount);
    bool ReadZPWRBCount(uint32_t& nCount);
