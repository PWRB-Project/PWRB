...
bool CLegacyZPwrbStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZPwrbStake from non zc-spend");
    ...
    *this = CLegacyZPwrbStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zpwrb stake origin", __func__);

    // All good
    return true;
}

CLegacyZPwrbStake::CLegacyZPwrbStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZPwrbStake::GetIndexFrom()
{
...
CAmount CLegacyZPwrbStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZPwrbStake::GetUniqueness() const
{
...
bool CLegacyZPwrbStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (nHeight < consensus.height_start_ZC_SerialsV2 || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zPWRB stake block: height %d outside range", __func__, nHeight);
