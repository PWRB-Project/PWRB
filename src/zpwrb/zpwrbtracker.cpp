...
#include "zpwrb/zpwrbwallet.h"


CzPWRBTracker::CzPWRBTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzPWRBTracker::~CzPWRBTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzPWRBTracker::Init()
{
...
bool CzPWRBTracker::Archive(CMintMeta& meta)
{
...
bool CzPWRBTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzPWRBTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzPWRBTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzPWRBTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzPWRBTracker::GetSerialHashes()
{
...
CAmount CzPWRBTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzPWRBTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzPWRBTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzPWRBTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzPWRBTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzPWRBTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzPWRBTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzPWRBTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzPWRBTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzPWRBTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzPWRBTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzPWRBWallet* zPWRBWallet)
{
    bool iszPWRBWalletInitialized = (nullptr != zPWRBWallet);
    CMintMeta meta;
    ...
    if (!iszPWRBWalletInitialized)
        zPWRBWallet = new CzPWRBWallet(wallet);
    meta.isSeedCorrect = zPWRBWallet->CheckSeed(dMint);
    if (!iszPWRBWalletInitialized)
        delete zPWRBWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzPWRBTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPWRBTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzPWRBTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzPWRBTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzPWRBTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzPWRBTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzpwrb from DB\n", __func__, listDeterministicDB.size());
    }
