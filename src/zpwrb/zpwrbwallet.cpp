...
CzPWRBWallet::CzPWRBWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zpwrb seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPWRBSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPWRBSeed_deprecated()) {
                    LogPrintf("%s: Updated zPWRB seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpwrb seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpwrb wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPWRBWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPWRBCount(nCountLastUsed);
    else if (!walletdb.ReadZPWRBCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPWRBWallet::Lock()
{
    seedMaster.SetNull();
}

void CzPWRBWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPWRBWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPWRB(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPWRBWallet::LoadMintPoolFromDB()
{
...
void CzPWRBWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPWRBWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPWRBWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zpwrbTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPWRBWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPWRB(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpwrbTracker which also adds to database
    wallet->zpwrbTracker->Add(dMint, true);
...
void CzPWRBWallet::SeedToZPWRB(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzPWRBWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPWRBWallet::GenerateDeterministicZPWRB(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPWRBWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPWRBWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzPWRBWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
