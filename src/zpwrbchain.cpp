...
{
    CDataStream serializedCoinSpend = ZPWRBModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZPWRBSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
    ...
    //Add mints to zPWRB supply (mints are forever disabled after last checkpoint)
    ...
    //Remove spends from zPWRB supply
    ...
    // A one-time event where only the zPWRB supply was off (due to serial duplication off-chain on main net)
...
bool UpdateZPWRBSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
    ...
    // Re-add spends to zPWRB supply
    std::list<libzerocoin::CoinDenomination> listDenomsSpent = ZerocoinSpendListFromBlock(block, true);
    for (const libzerocoin::CoinDenomination& denom : listDenomsSpent) {
        mapZerocoinSupply.at(denom)++;
    }

    // Remove mints from zPWRB supply (mints are forever disabled after last checkpoint)
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        std::list<CZerocoinMint> listMints;
        std::set<uint256> setAddedToWallet;
        BlockToZerocoinMintList(block, listMints, true);
