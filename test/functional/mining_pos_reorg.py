...
from test_framework.test_framework import PwrbTestFramework
...
class ReorgStakeTest(PwrbTestFramework):
    ...
    def check_money_supply(self, expected_pwrb, expected_zpwrb):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected PWRB and zPWRB supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_pwrb))
            for denom in node['zPWRBsupply']:
                assert_equal(node['zPWRBsupply'][denom], DecimalAmt(expected_zpwrb[denom]))
    ...
    def run_test(self):
        ...
        # Check PWRB and zPWRB supply at the beginning
        # ------------------------------------------
        # zPWRB supply: 2 coins for each denomination
        expected_zpwrb_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # PWRB supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zpwrb_supply)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zpwrb",
        ...
        # Verify that PWRB and zPWRB supplies were properly updated after the spends and reorgs
        self.log.info("Check PWRB and zPWRB supply...")
        ...
        expected_zpwrb_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zpwrb_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zpwrb_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zpwrb_supply)
