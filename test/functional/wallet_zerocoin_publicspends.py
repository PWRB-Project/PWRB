...
class ZerocoinSpendTest(PwrbTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zpwrb_bal, pwrb_bal):
            zpwrb_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zpwrb_bal)
            pwrb_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], pwrb_bal)
            return zpwrb_bal, pwrb_bal
        ...
        zpwrb_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zpwrb_balance, 6666)
        ...
        zpwrb_balance, balance = check_balances(denom_2, zpwrb_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zpwrb_balance, balance = check_balances(denom_3, zpwrb_balance, balance)
        ...
        _, _ = check_balances(denom_1, zpwrb_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
